...
### 2026-06-08 — 残機能の総実装（kitty パリティ・パス）

- **背景**: 監査（4エージェント並列でコードを精査）の結果、Issue #3〜#25 は全て
  クローズ済みだが、コード上は「解析のみ/スタブ/描画なし/配線漏れ」の項目が多数あった。
  ユーザー指示「未実装は全部やる。再監査でオールグリーンにする」。
- **実装した主な項目**（end-to-end）:
  - 描画: 拡張装飾(#24, SGR 4:0-5/58/59 をコロンsub-param対応で解析＋シェーダ描画)、
    リガチャ(#18)、カラー絵文字(#23)、kitty グラフィック画像描画(#15)、
    背景透過/ぼかし(#22)、同期更新+ダーティ行部分アップロード(#20)。
  - 入力: kitty キーボードプロトコル CSI u(#14)、URL タップ起動+下線+OSC8(#16)、
    ライブフォントリサイズ(#6)。
  - システム/UI: 通知 OSC99/777 配線(#19)、ベル(視覚/音/振動)(#21)、
    分割 findFocusedLeaf 修正+レイアウト切替(#11)、kitty.conf の include 再帰+
    解析のみキーの適用(#12)、テーマピッカー(#10)、shell統合のプロンプトジャンプ+
    スクリプト注入(#13)、スクロールバック検索 UI(#17)、結合文字の合成(#7)。
- **検証方針**: コンテナに Android SDK が無いため、Kotlin コンパイラ +
  Robolectric `android-all.jar` を取得し、emulator/render/font/input/config/helper
  パッケージを型チェック（合格）。結合文字・SGRコロン・下線色・ダーティ範囲は JVM で実動作確認。
  androidx に依存する view/activity 層は Google Maven が遮断され（ネットワークポリシー 403）
  ローカル型チェック不可のため、GitHub Actions の APK ビルドで最終検証。
- **CI トリガの制約**: App トークン由来の push では Actions が自動起動しないため、
  検証は PR 作成（opened イベント）で回す運用。
